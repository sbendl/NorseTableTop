...
class Sword:
    ...
    def foreswing(self, other, ang_velocity, body_len):
        cut_length = min(self.length, other.width, other.length)
        tip_velocity = ang_velocity * (body_len + self.length)
        bottom_velocity = ang_velocity * (body_len + (self.length * 2 / 3) - cut_length)
